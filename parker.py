...
def addentry(event=None):
    ...
    qso["qso_date"]=("{:04}{:02}{:02}".format(yearInt.get(),monthInt.get(),dayInt.get()))
    qso["time_on"]=("{:02}{:02}{:02}".format(hourInt.get(),minuteInt.get(),secondInt.get()))
    ...
    if p2pStr.get():
        qso["sig"]="POTA"
        qso["sig_info"]=p2pStr.get().upper()  # this does not validate the other station's reference
    else:
        pass
    ...
    if checkRequiredFields():
        if not p2pStr.get():
            # no p2p info
            qsotext=(datetimeString() + "QSO by {} on date {} at time {} at park {} with {} on band {} using mode {} comment {}".format(
            ...
            print(qsotext)
                ...
                # with p2p info
                qsotext=("QSO by {} on date {} at time {} at park {} with {} on band {} using mode {} their park {} comment {}".format(
...
def writeADIF():
    ...
    if len(logbook)>0:
        ...
        STARTDATETIME="{}-{}".format(logbook[0]['qso_date'],logbook[0]['time_on'])
        ...
        outfile.write("PROGRAMID:{}>{}\n".format(len(info['programid']),info['programid']))
        outfile.write("PROGRAMVERSION:{}>{}\n".format(len(info['programversion']),info['programversion']))
...
pL=Label(app,text="My Park").grid(row=0,column=1)
pE=Entry(app,textvariable=parkStr)
pE.grid(row=1,column=1)

yearL=Label(app,text="Year").grid(row=0,column=2)
yearE=Entry(app,width=4,textvariable=yearInt,state='disabled')
yearE.grid(row=1,column=2,sticky="ew")

monthL=Label(app,text="Month").grid(row=0,column=3)
monthE=Entry(app,textvariable=monthInt,width=2,state='disabled')
monthE.grid(row=1,column=3,sticky="ew")

dayL=Label(app,text="Day").grid(row=0,column=4)
#dayE=Entry(app,textvariable=dayInt,width=2,state='disabled')
dayE=Spinbox(app,textvariable=dayInt,from_=0,to=31,width=2,state='disabled')
dayE.grid(row=1,column=4,sticky="ew")

hourL=Label(app,text="Hour").grid(row=0,column=5)
hourE=Spinbox(app,textvariable=hourInt,from_=0,to=23,width=2,state='disabled')
hourE.grid(row=1,column=5,sticky="ew")

minuteL=Label(app,text="Minute").grid(row=0,column=6)
minuteE=Spinbox(app,textvariable=minuteInt,from_=0,to=59,width=2,state='disabled')
minuteE.grid(row=1,column=6,sticky="ew")

secondL=Label(app,text="Second").grid(row=0,column=7)
secondE=Spinbox(app,textvariable=secondInt,width=2,from_=0,to=59,state='disabled')
secondE.grid(row=1,column=7,sticky="ew")
...
bL.grid(row=0,column=8)
bands=OptionMenu(app,bandStr,*validBands,command=bandcallback)
bands.grid(row=1,column=8)

mL=Label(app,text='Mode')
mL.grid(row=0,column=9)
modes=OptionMenu(app,modeStr,*validModes,command=modecallback)
modes.grid(row=1,column=9)

bAdd=Button(app,text="Add QSO",command=addentry).grid(row=3,column=8)
bQuit=Button(app,text="Quit",command=goodbye).grid(row=3,column=9)
...
r1=Radiobutton(app,text="Live",variable=liveLogging,value=True,command=loggingModeCallback)
r1.grid(row=3,column=6)
r2=Radiobutton(app,text="Manual",variable=liveLogging,value=False,command=loggingModeCallback)
r2.grid(row=3,column=7)

blankRow=Label(app,text=" ")
blankRow.grid(row=6,column=0)

statusbar = Label(app, text="POTA Field Logger is ALPHA quality - Not ready for real use", bd=1, relief=SUNKEN, anchor=W)
statusbar.grid(row=7,column=0,columnspan=10,sticky="ew")
